...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figs/equipments/32course.pdf}
    \caption{実験装置全体図}
    \label{fig:32course}
\end{figure}
\begin{figure}[H]
...
\cref{fig:--}より、分離度を最大にするDelayの値は$50 \, \mathrm{ns}$であることが分かった。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/QDC2_50ns.pdf}
    \caption{Delay $50 \,\mathrm{ns}$時のQDC(Full) -- QDC(Tail) 相関.}
    \label{fig:QDC2_50ns}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/QDC2_yprojection.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/root/Al_TDCQDC.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/root/Al_TDCQDCn_full.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/root/Al_TDCQDCn.pdf}
...
% Time Callibratorによる時間校正の結果から、$a = 22.21\, \mathrm{[Ch./ns]}$であることを用いると、
% \begin{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/root/Al_TDCQDCg_full.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Ti_TDCQDC.pdf}
    \caption{\ce{Ti}標的におけるTDC--QDC相関.}
    \label{fig:Ti_TDCQDC}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Ti_TDCQDCn.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Ag_TDCQDC.pdf}
    \caption{\ce{Ag}標的におけるTDC--QDC相関.}
    \label{fig:Ag_TDCQDC}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Ag_TDCQDCn.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Au_TDCQDC.pdf}
    \caption{\ce{Au}標的におけるTDC--QDC相関.}
    \label{fig:Au_TDCQDC}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Au_TDCQDCn.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Li_TDCQDC.pdf}
    \caption{\ce{Li}標的におけるTDC--QDC相関.}
    \label{fig:Li_TDCQDC}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Li_TDCQDCn.pdf}
...
ここから、生成された$\gamma$線がシンチレータに届いてから次のRF信号が入力されるまでの時間が分かる。
Time Callibratorによる時間校正の結果から、TDCのチャンネルと時間の対応について、$a = 22.21\, \mathrm{[Ch./ns]}$であることを用いると、
\begin{equation}
    \mathrm{TDC}_\gamma = \frac{980 \, \mathrm{[Ch.]} }{22.21\, \mathrm{[Ch./ns]}} \approx 44.12\,\mathrm{[ns]}
\end{equation}
である。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figs/root/Al_TOFQDC.pdf}
...
このピークがクーロンエネルギー差に対応していると考えられる。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Al_Ex.pdf}
    \caption{\ce{Al}標的における中性子エネルギー分布.}
    \label{fig:Al_Ex}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Ti_Ex.pdf}
    \caption{\ce{Ti}標的における中性子エネルギー分布.}
    \label{fig:Ti_Ex}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Ag_Ex.pdf}
    \caption{\ce{Ag}標的における中性子エネルギー分布.}
    \label{fig:Ag_Ex}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Au_Ex.pdf}
    \caption{\ce{Au}標的における中性子エネルギー分布.}
    \label{fig:Au_Ex}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Li_Ex.pdf}
    \caption{\ce{Li}標的における中性子エネルギー分布.}
    \label{fig:Li_Ex}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Al_Ex_fitting.pdf}
...
このようなピークが見られる要因は、\ce{Li}標的の厚さが他の標的と比べ厚いため、\ce{Li}標的内部で陽子が反応する位置によっては入射エネルギーの減衰が生じるためである。
従って、このピークの左側が減衰なく反応したときのクーロンエネルギー差に相当する。
また、これによって理想的にはIAS励起エネルギーによってステップ状のピークが見られるが、後述するような種々の不定性によってガウス関数が畳み込まれたような、鈍ったピークとして観測されている。

そこで、このピークの左側のエッジについて\cref{fig:Li_Ex_fitting}のようにガウスフィッティングを行い、そのガウシアンの半値幅を基にクーロンエネルギー差を決定した。
このとき、ガウシアンの平均は$2.103 \pm 0.286\,\mathrm{[MeV]}$, 標準偏差は$0.6066 \pm 0.1225\,\mathrm{[MeV]}$であるから、半値幅の右側を取って、クーロンエネルギー差は$1.389 \pm 0.142\,\mathrm{[MeV]}$であると求まる。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figs/root/Li_Ex_fitting.pdf}
...
\begin{table}[H]
    ...
    \begin{tabular}{cc}
        \hline
         ...
         \hline\hline
         \ce{Al} & $5.684 \, \pm \, 0.02544$\\
         \ce{Ti} & $7.965 \, \pm \, 0.04620$\\
         \ce{Ag} & N/A\\
         \ce{Au} & $18.43 \, \pm \, 0.03669$\\
         \ce{Li} & $1.389 \,\pm \, 0.1420$ \\
         \hline 
    \end{tabular}
    \caption{Caption}
...
\begin{table}[H]
    ...
    \begin{tabular}{cc}
        ...
        標的原子 &  原子核半径 $\mathrm{[fm]}$\\
         \hline\hline
         \ce{Al} & $4.104 \, \pm \, 0.018$\\
         \ce{Ti} & $4.881 \, \pm \, 0.028$\\
         \ce{Ag} & N/A\\
         \ce{Au} & $7.453 \, \pm \, 0.015$\\
         \ce{Li} & $3.425\,\pm \, 0.2755$ \\
         \hline 
    \end{tabular}
    \caption{Caption}
...
ここで、原子核半径$R$と質量数$A$には$R = A^{1/3}$の関係がある。
そこで、$A^{1/3}$と$R$の相関を次の\cref{fig:cbrt_A_vs_R}に示す。
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{figs/graph/nucleus_radius_vs_A_cbrt.pdf}
    \caption{Caption}
    \label{fig:cbrt_A_vs_R}
\end{figure}

不定性の大きい\ce{Li}を除いた\ce{Al}、\ce{Ti}、\ce{Au}の原子については、線形の関係になっていることが分かった。
...
\cref{fig:シンチレータ}に示すように、液体シンチレータは中性子の進行方向に対して$50.8\,\mathrm{mm}$の幅をもつ。
したがって、ターゲットからシンチレータまでの距離には$\pm 25.4\,\mathrm{mm}$の位置不確定性が存在すると考えられる。

この位置の誤差のクーロンエネルギー差$E_{\text{c}}$および原子半径$R$への伝搬の過程を以下に示す。
...
\begin{enumerate}
    ...
    \begin{align*} 
        ...
        &= \frac{m_n \beta}{(1-\beta^2)^{\frac{3}{2}}} \cdot \frac{d L}{tc}
    \end{align*}
...
\begin{table}[H]
    ...
    \begin{tabular}{c|cc|cc}
        ...
        \cline{2-5}
         & $+$側 & $-$側 & $+$側 & $-$側 \\
        \hline\hline
        \ce{Al} & $0.313$ & $0.311$ & $0.226$ & $0.225$ \\
        \ce{Ti} & $0.922$ & $0.294$ & $0.565$ & $0.180$ \\
        \ce{Au} & $0.387$ & $0.215$ & $0.157$ & $8.69 \times 10^{-2}$ \\
        \hline
    \end{tabular}
    \label{tab:err_exp}
\end{table}
...
\begin{align*}
    d(\Delta E_\text{c}) &= \pm 3.44 \times 10^{-1} \, [\mathrm{MeV}] \\
    dR &= 1.08 \, [\mathrm{fm}]
\end{align*}

解析結果と二乗和平方根を用いて合成し、$Ag$を除く各ターゲットの誤差をまとめた結果を\cref{tab:err_all}に示す。
...
\end{align*}
と求められる。このとき、\cref{ビームダンプ}を見るとターゲットからビームダンプまでの距離は約$2.1\,\mathrm{m}$であるため、$T_\text{p} = 50 \,\mathrm{MeV}$と仮定すると飛行時間は
\begin{equation*}
...
\paragraph{荷電粒子の信号}
TDC--ADC相関図の中で、TDCおよびADCの両方で明瞭なピークを示す信号（\cref{fig:Al_TDC-ADC_separated}の②）は、荷電粒子によるものであると考えられる。荷電粒子はBetheの式（\cref{betheの式}）に示されるように、ガンマ線や中性子に比べシンチレータ中で大きなエネルギー損失を生じ、ADC値が明瞭なピークを形成するためである。また、この信号は$T_\text{RF}$と同周期で出現することから、陽子ビームと同期して放出されていることがわかる。

\cref{ビームダンプ}に示す実験配置を考慮すると、遮蔽物を通過しにくい荷電粒子が検出器に到達する場合、主にターゲット側から入射したものに限られる。したがって、この信号は陽子入射に伴って生成された粒子である可能性が高い。ただし、本実験のデータのみから粒子種を特定することは困難であると考えられる。
