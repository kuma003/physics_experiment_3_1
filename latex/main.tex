...
%
\begin{abstract}
    本実験では、アイソバリックアナログ状態（IAS）の観測を通じて原子核半径の推定を行った。ターゲットとしてAl、Ti、Ag，Au，Liを用い、約50MeVの陽子ビームを入射した。中性子の飛行時間を測定することでIASの励起エネルギーを算出し、そこから原子核半径を推定した。その結果，\ce{Ag}を除く各ターゲットに対して以下の原子核半径を得ることができ，実験の目的である原子半径の推定を達成したと言える。Al: 3.07±0.12 fm、Ti: 3.59±0.14 fm、Au: 5.23±0.20 fm、Li: 2.43±0.10 fm。
\end{abstract}
...
\section{目的}
本実験の目的はアイソバリックアナログ状態（IAS）の観測を通じて原子核半径の推定をすることである。
...
\begin{equation}
    ...
    \label{eq:励起エネルギー2}
\end{equation}
となる。$m_p,\quad m_n$は文献値を用い、$T_p$は加速器の出射エネルギーを代入する。運動エネルギーは式\ref{eq:運動エネルギー}のように与えられ、式中の$\beta$が分かれば$T_n$を計算できる。
\begin{align}
    T_n&=(\gamma-1)m_nc^2\notag\\
    &=m_nc^2\left(\frac{1}{\sqrt{1-\beta^2}}-1\right)
\label{eq:運動エネルギー}
\end{align}
    ここで、中性子の速度$\beta c$と飛行距離$L$、および飛行時間$\Delta t$には以下の関係がある。
    \begin{equation}
        \beta c=\frac{L}{\Delta t}
    \end{equation}
    本実験では、飛行時間$\Delta t$を測定することで運動エネルギーを算出する。飛行時間の測定には加速器の特性を利用する。加速器は一定周期で加速しており、加速のタイミングに合わせて通称「RF信号」と呼ばれる信号が出ている。図\ref{fig:飛行時間}のように、標的からは確率的に中性子か$\gamma$線が放出されている。$\gamma$線が検出されてから直後のRF信号までの時間を$\text{TDC}_{\gamma}$、中性子の場合を$\text{TDC}_n$とし、両者が検出された直後のRF信号は何周期か差があったとする。このとき、飛行時間は
    \begin{align}
        \Delta t&=\frac{L}{c}+\text{TDC}_{\gamma}+k\times T_{RF}-\text{TDC}_n\\
        (k&=0,1,2\cdots)
    \end{align}
    と書ける。
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{figs/misc/TOF.pdf}
    \caption{飛行時間の測定についての概念図。RF信号を用いて$\Delta t$を決定する。}
    \label{fig:飛行時間}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figs/misc/PSD.png}
    \caption{PSDにおける中性子とγ線の弁別}
    \label{fig:placeholder}
\end{figure}
上図のとおり液体シンチレータで観測される波形は、中性子とγ線で異なる。出力シグナルの時間積分、つまり電荷$Q$について、$Q(\mathrm{Tail})/Q(\mathrm{Full})$の値が大きい方が中性子、$Q(\mathrm{Tail})/Q(\mathrm{Full})$の値が小さい方がγ線である。\\
...
これはつまり陽子の通過軌跡周辺の狭い領域で励起分子の密度が大きくなることを意味する。このことから励起分子同士で相互作用が起こり、スピン三重項状態が生成される割合が大きくなる。そして2つの三重項状態の分子同士での衝突（三重項消滅）が起こりやすくなり、一方の分子は基底状態にもう一方の分子は励起一重項状態に遷移する。その後、後者の分子が基底状態に脱励起する際に蛍光を発し、その光が観測されるパルスの「遅い成分」として「多く」現れる。

\section{実験}
\subsection{実験セットアップ全体図、回路図}

実験装置の全体像を図\ref{fig:32course}に示す。図の左に加速器があり、加速された陽子が右方へ入射してくる。加速の周期の逆数（RF周波数）は61.6nsであった。陽子ビームが標的に入射すると原子核反応によって中性子や$\gamma$線が放射される。それらをビーム軌道上に設置した液体シンチレータで検出する。反応せずに標的を透過した陽子は電磁石によって軌道を曲げてビームダンプで吸収する。飛行距離は$L=7507[\text{mm}]$であった。
\begin{figure}[H]
    \centering
    ...
    \label{fig:32course}
\end{figure}
液体シンチレータの構造を図\ref{fig:liquid_scintillator}に示す。
\begin{figure}[H]
    \centering
...
\end{figure}
標的には5種類の金属を用いた。それぞれの元素記号とビーム入射方向の厚さを表\ref{tab:targetthickness}に示す。
\begin{table}[H]
    \centering
    \begin{tabular}{cc}
    \hline
        ...
        \hline
    \end{tabular}
    \caption{標的の種類と厚さ。他に比べ、Li標的が厚くなっている。}
    \label{tab:targetthickness}
\end{table}
本実験で用いた回路図を図\ref{fig:circuit}に示す。
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{figs/misc/circuit.pdf}
    \caption{実験に用いた回路の概念図。}
    \label{fig:circuit}
\end{figure}
...
ターゲット内部でのエネルギー減衰の大きさはターゲット厚さに依存するため、\cref{tab:targetthickness}に示した各ターゲット厚さをもとに、入射陽子がターゲット表面から通過した場合に失う最大の減衰エネルギー$E_\text{d}$をLISE++を用いて算出した。
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=0.8\linewidth]{figs/graph/Al_TDCQDC_separated_comp.pdf}
